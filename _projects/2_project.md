...
### 4. Find the 3 minutes of the pass that includes the storm, then convert time to distance to get the greatcircle distance between shots
...
### 5. Make the plot for rain rate and reflectivity
{% raw %}
```python
...
```
{% endraw %}
### 8. ECMWF temperatures for the segment
{% raw %}
```python
...
### 10. Split out the long and shortwave heating rates
{% raw %}
```python
...
### 11. Make a plots for shortwave and longwave heating rates which look like so:
